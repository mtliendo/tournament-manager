...
export const pairSwiss = (tournament) => {
  var { players, numRounds, games, currentRound } = tournament;
  ...
  var groups = groupByScore(players);
...
export const addGamesToPlayers = (players, games) => {
  for (let i = 0; i < players.length; i++) {
    ...
    players[i] = {...players[i], colorCount, dueColor, opponents};
  }
  return players;
}
...
export const groupByScore = (players) => {
  var groups = [];
  var currentGroup = [];
  var groupScore = players[0].score;
  for (let i = 0; i < players.length; i++) {
...
const pairGroup = (groupNum, groups) => {
  var groupSize = groups[groupNum].length;
  var currentGroup = groups[groupNum];
  var oddPlayer;
  var nextGroup = [];
  ...
  var numGames = currentGroup.length / 2;
  var topHalf = [];
  var bottomHalf = [];
...
const pairRoundOne = (players) => {
  ...
  var nextPlayerColor = Math.floor((Math.random() * 2)) ? 'W' : 'B';
  for (let i = 0; i < numGames; i++) {
